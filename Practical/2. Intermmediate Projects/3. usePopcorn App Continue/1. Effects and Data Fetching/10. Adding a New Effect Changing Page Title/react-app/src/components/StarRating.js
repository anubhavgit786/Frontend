...
const Star = ({ onRating, full, onHoverIn, onHoverOut, color, size })=>
{
  ...
  return (<span role='button' style={starStyle} onClick={onRating} onMouseEnter={onHoverIn} onMouseLeave={onHoverOut} >{  full ? <FullStar color={color} /> : <EmptyStar color={color} /> }</span>)
}
...
const StarRating = ({ maxRatings = 5, color = '#fcc419', size = 48, messages=[], defaultRating=0, onSetRating }) => 
{
    ...
    <div style={containerStyle}>
        <div style={starContainerStyle}>{stars.map((star)=> <Star key={star} onRating={()=> handleRating(star)} onHoverIn={()=> handleHoverRating(star)} onHoverOut={()=> handleHoverRating(0)} full={ hoverRating ? hoverRating >= star : rating >= star } color={color} size={size} />)}</div>
        <p style={textStyle}>{ messages.length === maxRatings ? messages[ hoverRating ? hoverRating-1 : rating-1 ] : hoverRating || rating || "" }</p>
    </div>
  );
}
