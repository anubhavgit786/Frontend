...
const Star = ({ onRating, full })=>
{
  return (<span role='button' style={starStyle} onClick={onRating} >{  full ? <FullStar/> : <EmptyStar/> }</span>)
}
...
const StarRating = ({ maxRatings = 5 }) => 
{
    ...
    <div style={containerStyle}>
        <div style={starContainerStyle}>{stars.map((star)=> <Star key={star} onRating={()=> handleRating(star)} full={ rating >= star } />)}</div>
