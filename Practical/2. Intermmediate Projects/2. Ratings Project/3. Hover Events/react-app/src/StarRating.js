...
{
  return (<span role='button' style={starStyle} onClick={onRating} onMouseEnter={onHoverIn} onMouseLeave={onHoverOut} >{  full ? <FullStar/> : <EmptyStar/> }</span>)
}
...
const StarRating = ({ maxRatings = 5 }) => 
{
    ...
    <div style={containerStyle}>
        <div style={starContainerStyle}>{stars.map((star)=> <Star key={star} onRating={()=> handleRating(star)} onHoverIn={()=> handleHoverRating(star)} onHoverOut={()=> handleHoverRating(0)} full={ hoverRating ? hoverRating >= star : rating >= star } />)}</div>
